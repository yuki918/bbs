          ...
          <div class="">
            @if(count($user->thread) > 0)
              @foreach($user->thread as $userThread)
                <a href="{{route('thread.show', ['thread' => $userThread->id])}}" class="block px-2 py-4 border-b border-black hover:opacity-80 hover:bg-gray-100 transition-all">
                  ...
                  <p class="mt-1">{{$userThread->first_comment}}</p>
                </a>
              @endforeach
            @else
          ...
          <div class="">
            @if(count($threads) > 0)
              @foreach($threads as $thread)
                <a href="{{route('thread.show', ['thread' => $thread->id])}}" class="block px-2 py-4 border-b border-black hover:opacity-80 hover:bg-gray-100 transition-all">
                  <h2>{{$thread->title}}</h2>
                  <div class="flex mt-1 flex-wrap md:flex-nowrap">
